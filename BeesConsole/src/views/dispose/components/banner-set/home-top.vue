...
export default {
  ...
  data() {
    return {
      ...
      bannerDisplayed: 0,
      src: require('@/assets/img/顶部banner.jpeg')
    }
  },
  computed: {
    ...
    handleSearch(val) {
      ...
      request({
        ...
        data: {
          ...
          size: this.query.size,
          bannerLocation: 0,
          bannerStartTime: val.optionDate.length > 0 ? val.optionDate[0] : null,
          bannerEndTime: val.optionDate.length > 0 ? val.optionDate[1] : null,
          bannerToDTO: {
            bannerToId: val.issue === 0 ? val.issue : null
          },
          bannerDisplayed: (val.state === 0 || val.state) ? val.state : null,
          bannerRegionDTOS:
