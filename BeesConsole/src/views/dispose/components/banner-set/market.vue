  ...
  <div class="market">
    ...
    >
      ...
      <!-- <el-table-column
        ...
        prop="bannerProductName"
        label="商品名称"
        align="center"
      />
      <el-table-column
        show-overflow-tooltip
        prop="bannerProductId"
        label="商品政策编码"
        align="center"
      >
        <template slot-scope="{row}">
          {{ row.bannerProductId.split('-')[0] }}
        </template>
      </el-table-column>
      <el-table-column
        show-overflow-tooltip
        prop="bannerActivityType"
        label="商品"
        align="center"
      /> -->
      ...
      >
        <template slot-scope="{row}">
          {{
            (isBannerLocation===6 && bannerForwardType===3) ? row.bannerProductName : row.bannerActivityName
          }}
        </template>
      </el-table-column>
      ...
      <el-table-column
        show-overflow-tooltip
        :prop="(isBannerLocation===6 && bannerForwardType===3) ? 'bannerProductId' : 'bannerActivityCode'"
        label="活动编号"
        align="center"
      >
        <template slot-scope="{row}">
          {{
            (isBannerLocation===6 && bannerForwardType===3) ? row.bannerProductId : row.bannerActivityCode
          }}
        </template>
      </el-table-column>
      <el-table-column
...
export default {
  ...
  computed: {
    ...
    isBannerLocation() {
      return this.$route.params.bannerLocation
    },
    bannerForwardType() {
      return this.$route.params.bannerForwardType
    }
  },
  watch: {},
  created() {
    this.getMarke()
    console.log(this.isBannerLocation, this.bannerForwardType)
  },
  mounted() {},
  beforeCreate() {},
  beforeMount() {},
